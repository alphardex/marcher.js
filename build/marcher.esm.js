...
class PrimitiveSDF {
    ...
    getTranslateShader(x, y, z) {
        return `${this.pointVarName}+=vec3(${toFixed2(x)},${toFixed2(y)},${toFixed2(z)});`;
    }
    ...
    getRotateShader(deg, axis) {
        return `${this.pointVarName}=rotate${axis.toUpperCase()}(${this.pointVarName},${toFixed2(deg2rad(deg))});`;
    }
    rotate(deg = 0, axis = "x") {
...
export { BezierSDF, BoxSDF, CylinderSDF, DEFAULT_MATERIAL_ID, GroupSDF, GyroidSDF, JointSDF, Marcher, PolygonSDF, PrimitiveSDF, RhombusSDF, SDFLayer, SDFMainImage, SDFMapFunction, SDFMaterial, SDFRender, SphereSDF, TriangleSDF, UberprimBoxSDF, UberprimCapsuleSDF, UberprimConeSDF, UberprimCylinderSDF, UberprimSDF, UberprimSphereSDF, UberprimTorusSDF, compact, deg2rad, joinLine, lerp, rad2Deg, reverse, toFixed1, toFixed2 };
